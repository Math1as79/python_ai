...
def __generate_breakout_labels(df_breakout, oc_vs_ma = 100, volume_vs_ma = 50):
    df_breakout['Breakout'] = (df_breakout['Open_Close'] >= 0.0) & (df_breakout['Open_Close'] == df_breakout['MaxOC_Prev10']) & (df_breakout['OC-%-MA20'] >= oc_vs_ma) & (df_breakout['Volume-%-MA20_Volume'] >= volume_vs_ma) 
    return df_breakout
...
def run_breakout(ticker):
    ...
    # Future consideration to look at Moving average 50 days as well
    ...
    # Setting them more generous now so that we can test the price prediction
    df_breakout = __generate_breakout_labels(df_breakout, 50, 20)
    ...
    if accuracy > 0.75:
        # Saving the model, so that going foward it might be utilized in price prediction
